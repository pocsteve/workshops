...
tomcat_permissions = node['tomcat']['permissions']
...
bash 'extract tomcat archive' do
  ...
  code <<-EOH
    tar xvf /tmp/apache-tomcat-8*tar.gz -C #{tomcat_dir} --strip-components=1
    EOH
...
ruby_block 'set recursive ownership & permissions' do
  block do
    ...
    FileUtils.chmod_R(tomcat_permissions, "#{tomcat_dir}/conf")
